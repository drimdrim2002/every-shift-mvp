...
| `admin_approve`      | super reviewer | target request is `pending` and role is `admin`          | `UPDATE signup_requests SET status='approved', reviewed_by, reviewed_at`                                                              | request becomes terminal `approved`                                                      |
| `admin_reject`       | super reviewer | target request is `pending` and role is `admin`          | `UPDATE signup_requests SET status='rejected', reviewed_by, reviewed_at, review_note`                                                 | request becomes terminal `rejected`                                                      |
...
| Event                | Actor              | Preconditions                                          | DB Writes                                                                                                              | Postconditions                                     |
| :------------------- | :----------------- | :----------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------- |
| `admin_approve`      | super reviewer     | corresponding admin `signup_requests` row is `pending` | `UPSERT organization_memberships(organization_id, user_id, role='admin', status='approved', approved_by, approved_at)` | requester has approved admin membership            |
...
| `admin_reject`       | super reviewer     | admin request is `pending`                             | membership must not transition to `approved` from this event                                                           | no active approved membership created by reject    |
...
3. Invite single-use:
   - Invite row can be consumed exactly once (`used_count < max_uses` -> `used_count = max_uses`).
   - `used_at` and `used_by` are compatibility integrity fields and must remain aligned with the consumed state.
...
1. Validate invite is active (`used_count < max_uses`, `revoked_at is null`, and `expires_at > now()`).
2. Mark invite consumed (`used_count = used_count + 1`, `used_at`, `used_by`).
