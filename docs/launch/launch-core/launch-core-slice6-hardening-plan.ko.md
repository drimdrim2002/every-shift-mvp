...
**목표:** 커스텀 도메인을 아직 구매하지 않았고 Vercel 프로젝트도 아직 구성되지 않은 상태에서도 실행할 수 있도록 Slice 6을 재정의합니다.

**아키텍처:** Slice 6을 저장소가 책임지는 배포 준비, Vercel 프로젝트 부트스트랩, Preview smoke 검증, Production 승격, 지연된 커스텀 도메인 설정으로 나눕니다. 코드 slice는 도메인이 없어도 merge 가능해야 합니다. 첫 배포 대상은 Vercel이 생성한 Preview/Production URL이며, 커스텀 도메인은 준비 완료 이후의 launch-ops 게이트로 둡니다.
...
- 커스텀 도메인 준비가 암묵적으로 포함되어 있지만 도메인은 아직 구매되지 않음
- 저장소가 책임지는 게이트와 외부 운영 설정이 섞여 있음

Slice 6은 구매한 도메인 때문에 막히면 안 됩니다. 먼저 저장소가 Vercel 생성 URL로 안전하게 배포될 수 있음을 증명한 뒤, 커스텀 도메인 설정을 명시적인 후속 게이트로 만들어야 합니다.
...
- 수정: `docs/launch/launch-core/launch-core-auth-and-deploy-spec.md`
  - Vercel 생성 URL이 최초 필수 대상이며, 커스텀 도메인은 구매 전까지 지연된다는 점을 명확히 합니다.
...
```text
저장소를 배포 가능한 상태로 만들고, 최초 Vercel preview 경로를 부트스트랩하며, 구매한 커스텀 도메인을 요구하지 않고 최종 런칭 회귀 게이트를 정의합니다.
```
...
- 아직 커스텀 도메인을 구매하지 않았음
- 아직 Vercel 프로젝트가 존재하지 않을 수 있음
- 첫 검증 대상은 Vercel 생성 URL임
- 커스텀 도메인과 DNS는 외부 launch-ops 작업임
- Slice 6 코드/문서는 커스텀 도메인이 존재하기 전에 merge 가능함
...
- [ ] **Step 1: 커스텀 도메인 전 production 대상 정의**

다음을 사용합니다.

```text
Production URL before domain purchase: https://<vercel-project>.vercel.app
```
...
```text
production deployment는 생성된 Vercel domain에서 검증할 수 있습니다. custom-domain launch는 domain purchase, DNS, SSL이 완료될 때까지 계속 blocked 상태입니다.
```
...
- domain 구매
- Vercel project에 domain 추가
- Vercel 안내에 따라 registrar DNS record 구성
- Vercel SSL certificate가 valid 상태가 될 때까지 대기
- custom domain에서 `/`, `/app`, `/login`, `/signup`, `/access/*`, 그리고 하나의 `/app/schedule/*` hard refresh를 smoke test
...
```text
custom domain purchase 때문에 Slice 6 repo completion을 block하지 않습니다. 대신 이 checklist로 public custom-domain launch를 block합니다.
```
