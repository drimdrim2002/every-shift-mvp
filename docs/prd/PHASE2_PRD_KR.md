...
- 동일 월에는 복수의 후보안(version)을 만들 수 있다.
- off 요청, 고정 배정, 정책, 입력 조건이 실질적으로 달라지면 새로운 version으로 본다.
- 같은 version 안에서 재생성, 수동 수정, 재검증은 revision 증가로 관리한다.
...
- 증명 스냅샷은 선택된 version의 현재 revision 기준으로 저장되어야 한다.
- 최종 확정은 선택된 version의 현재 revision에 대해 생성된 최신 passed evaluation 기준으로만 이루어져야 한다.
...
#### D. 후보안(version) 비교 리포트

같은 대상 월에 대해 생성된 여러 후보안(version)을 비교하고, 그중 하나를 최종 확정 대상으로 선택한다.

비교 조건:

- 동일 조직/병동
- 동일 대상 월
- 각 version의 변경 입력(off 요청, 고정 배정, 정책, 수동 수정 등)이 명시적으로 기록되어야 한다.
- compare의 목적은 하나의 최종 확정 대상 version을 선택하는 것이다.
...
- Phase2A의 핵심 compare 기능은 수기 기준안 입력을 기본 전제로 하지 않는다.
...
- 운영자가 선택된 version을 수동 수정한 경우 상태는 `review_pending`으로 전이되어야 하며, 증명 및 설명 결과를 다시 갱신해야 한다.
- 결과 확정은 선택된 version의 최신 passed evaluation이 해당 version의 현재 revision과 일치할 때만 가능해야 한다.
- 확정되지 않은 다른 version은 비교 대상으로 유지할 수 있어야 한다.
...
- 하드 제약 위반 0건
- 후보안 간 비교 시 선택된 version의 장단점을 운영자가 이해하고 선택할 수 있음
...
- 하나의 schedule container 아래에 여러 후보 version이 존재할 수 있다.
- 각 version은 여러 revision을 가질 수 있다.
...
- finalization은 월 전체가 아니라 선택된 version 단위로 수행한다.
- finalization은 `selected version + current revision + latest passed evaluation`이 모두 일치할 때만 허용한다.
- finalized version만 운영 확정본으로 간주하며, rolling fairness ledger 적재도 finalized version 기준으로 수행한다.

#### D. compare 규칙

- compare의 기본 단위는 수기 기준안이 아니라 후보 version이다.
- compare 화면은 version 간 입력 차이와 결과 차이를 함께 보여줘야 한다.
- 운영자는 compare 후 하나의 version을 선택해 finalization해야 한다.
...
## 6. 후보안(version) 비교 리포트 템플릿

다음 리포트는 실제 병동 파일럿에서 기본적으로 제공되어야 한다.

### 6.1 리포트 목적

- 동일 월에 대해 생성한 여러 후보안 중 어떤 version을 확정할지 선택 근거 제공
- 공정성, 요청 반영률, 하드 제약 상태를 version 단위로 비교
...
| Version | 생성 방식 | 입력 변경 요약      | 평가 상태      | 확정 가능 여부 |
| ------- | --------- | ------------------- | -------------- | -------------- |
| V1      | 초기 생성 | 기본 off 요청 기준  | review_ready   | 가능           |
| V2      | 재생성    | 일부 off 요청 조정  | review_ready   | 가능           |
| V3      | 재생성    | 필요 인원 요구 수정 | review_blocked | 불가           |

#### 비교 요약

| 지표                | V1    | V2    | V3    | 선택안 |
| ------------------- | ----- | ----- | ----- | ------ |
| 하드 제약 위반 건수 | 0     | 0     | 2     | V2     |
| off 요청 반영률     | 72%   | 81%   | 79%   | V2     |
| 야간 근무 최소/최대 | 0 / 7 | 4 / 5 | 3 / 6 | V2     |
| 주말 근무 최소/최대 | 1 / 6 | 3 / 4 | 2 / 5 | V2     |
| 수동 수정 건수      | 0건   | 1건   | 0건   | V2     |

#### 입력 차이 상세

| Version | 변경된 off 요청 | 변경된 정책 | 변경된 고정 배정 | 비고          |
| ------- | --------------- | ----------- | ---------------- | ------------- |
| V1      | 없음            | 없음        | 없음             | 초기안        |
| V2      | 2건 조정        | 없음        | 없음             | 요청 현실화   |
| V3      | 없음            | 1건 변경    | 없음             | staffing 실험 |

#### 해석 요약

- 운영자는 version 간 입력 차이와 결과 차이를 함께 보며 최종 확정 대상을 선택한다.
- `review_blocked` version은 compare 대상에는 남길 수 있으나 finalization 대상이 될 수 없다.
- 선택된 version은 최신 passed evaluation 기준으로만 finalization 가능하다.
...
- version compare에서 어떤 입력 차이(diff)를 기본 표시 항목으로 고정할 것인가
