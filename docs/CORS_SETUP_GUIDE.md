...
Vercel Preview/Production 배포에서는 Cloud Run을 브라우저에서 직접 호출하지 말고, `api/[...path].js` 의 `/api/*` same-origin function proxy를 우선 사용합니다. 이 문서는 Cloud Run을 직접 호출해야 하는 로컬/디버그 상황의 백엔드 CORS 설정 가이드입니다.
