...
> **상태:** 🟡 deep-interview 분석 완료 (UX 방향 확정 대기)  
> **작성일:** 2026-06-14  
...
### Q1. “같은 row”의 UI 의미 (최우선)

| 옵션         | 설명                                                                       | 장점                                                 | 단점                                                                     |
| ------------ | -------------------------------------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------ |
| **A (추천)** | 그리드에서 짝을 **인접한 두 행**으로 배치 + 짝 시각 표시(🔗, 배경, 연결선) | 30×36 구조 유지, 셀 클릭·Off 셀 독립, 구현 난이도 중 | 여전히 2행 (진짜 1행은 아님)                                             |
| **B**        | 그리드 **1행에 두 명** 이름·Off 셀 병합                                    | “같은 row” 문자 그대로                               | 셀 소유자 모호, TanStack/ScheduleGrid 대규모 변경, 30명 한도와 충돌 가능 |
| **C**        | 그리드 순서 유지, **드로어·요청 목록**만 짝 단위 표시                      | 변경 범위 최소                                       | 캘린더 스크롤 시 짝이 여전히 멀리 있음                                   |

**추천:** **A** — 업무 맥락(같은 날 Off를 나란히 확인)과 구현 리스크의 균형이 가장 좋음.

### Q2. 근무자 선택 시 짝 자동 포함 여부 (A 선택 시 후속)

| 옵션          | 설명                                                                                        |
| ------------- | ------------------------------------------------------------------------------------------- |
| **A1 (추천)** | 한쪽 선택·그리드 셀 클릭 시 **짝도 `selectedEmployeeIds`에 자동 추가** (동기화는 기존 로직) |
| **A2**        | 시각적 인접 배치만, 선택은 사용자가 각각                                                    |
...
| #   | 항목                                | 상태                                                  |
| --- | ----------------------------------- | ----------------------------------------------------- |
| 1   | “같은 row” = A / B / C              | 🟡 **사용자 결정 대기**                               |
| 2   | 선택 시 짝 자동 포함 (A1 vs A2)     | 🟡 A 확정 후 결정 (추천 A1)                           |
