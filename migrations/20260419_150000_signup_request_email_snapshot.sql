-- Store signup requester emails as request-time snapshots for approval reads.

ALTER TABLE IF EXISTS public.signup_requests
ADD COLUMN IF NOT EXISTS requester_email text;

UPDATE public.signup_requests sr
SET requester_email = nullif(trim(au.email), '')
FROM auth.users au
WHERE sr.requester_user_id = au.id
  AND nullif(trim(coalesce(sr.requester_email, '')), '') IS NULL
  AND nullif(trim(coalesce(au.email, '')), '') IS NOT NULL;

DROP FUNCTION IF EXISTS public.submit_admin_signup_atomic(uuid, uuid, text, text);
...
DECLARE
  ...
  v_invite_id uuid;
  v_invite_role_scope text;
  v_invite_expires_at timestamptz;
  v_invite_max_uses integer;
  v_invite_used_count integer;
...
BEGIN
  ...
  IF v_invite_role_scope <> 'user' THEN
  ...
  IF v_invite_expires_at <= v_now THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'invalid_invite_code_expired';
  END IF;

  IF COALESCE(v_invite_used_count, 0) >= GREATEST(COALESCE(v_invite_max_uses, 1), 1)
    OR v_invite_used_at IS NOT NULL
    OR v_invite_used_by IS NOT NULL THEN
...
REVOKE ALL ON FUNCTION public.redeem_user_invite_signup_atomic(uuid, uuid, text, text, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.redeem_user_invite_signup_atomic(uuid, uuid, text, text, text) TO service_role;
