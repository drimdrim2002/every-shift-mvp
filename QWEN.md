...
**CRITICAL**: Supabase MCP is configured in **READ-ONLY** mode globally.

**Read Operations** (via MCP allowed):

- `mcp__supabase__list_tables`, `mcp__supabase__list_migrations`
- `mcp__supabase__execute_sql` with SELECT queries only (when MCP enabled)

**Write Operations** (API Layer ONLY):

- NEVER use MCP for INSERT/UPDATE/DELETE
- ALWAYS use API functions in `src/api/` directory
- Example: Use `createSchedule()` from `src/api/schedule.ts`

**Reference**: See `docs/MCP_ACCESS.md` for complete MCP rules
